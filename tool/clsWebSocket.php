...
class clsWebSocket {
    ...
    public function __construct($host, $port) {
        try {
            ...
            // listen函数使用主动连接套接口变为被连接套接口, 使得一个进程可以接受其他进程的请求
            // 从而成为一个服务器进程. 在tcp服务器编程中listen函数把进程变为一个服务器, 并指定相应的套接字变为被动连接,
            // 其中的能存储的请求不明的socket数目
    ...
    private function doServer() {
        ...
        foreach ($sockets as $socket) {
            ...
            if ($socket == $this->master) {
                ...
                // 创建, 绑定, 监听后accept函数将会接受socket要来的链接, 一旦有一个连接成功, 将会返回一个新的socket资源用以交互,
                // 如果是一个多个连接的队列, 只会处理第一个, 如果没有连接的话, 进程将会被阻塞, 直到连接上.
                // 如果set_socket_blocking或socket_set_noblock()设置了阻塞, 会返回false;
                // 返回资源后, 将会持续等待连接.
                if (false === $client) {
                    $this->error([
                        'err_accept',
                ...
                if ($bytes < 9) {
                    ...
                    if (!$this->sockets[(int)$socket]['handlehake']) {
                        self::handleShake($socket, $buffer);
                        continue;
                    } else {
    ...
    public function handShake($socket, $buffer) {
        ...
        $this->sockets[(int)$socket]['handleshake'] = true;
     ...
     * @return string
     */
    private function deakMsg($socket, $recv_msg) {
